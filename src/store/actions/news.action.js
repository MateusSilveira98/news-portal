...
export function getNewsByPage(currentPage) {
  return dispatch => {
    axios.get(`https://content.guardianapis.com/search?api-key=0d160d0f-71cd-48b0-801f-2fc9cabd2157&page=${currentPage + 1}`)
      .then(response => {
        dispatch(updateNews(response.data.response));
      })
      .catch(error => {
